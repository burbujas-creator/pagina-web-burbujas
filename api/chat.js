...
export default async function handler(req, res) {
  ...
  try {
    ...
    function estadoLocalAhora() {
      ...
      const dia = diaRaw.normalize("NFD").replace(/[\u0300-\u036f]/g, ""); // quita acentos

      const habil = ["lunes","martes","miercoles","jueves","viernes","sabado"].includes(dia);
      const dentroDeHorario = (hora > 8 && hora < 21) || (hora === 8 && minuto >= 0);
      return (habil && dentroDeHorario) ? "abierto" : "cerrado";
    }
    const estadoAhora = estadoLocalAhora();

    // ---------- ENTRENAMIENTO (tu bloque + runtime) ----------
    const sistema = `
...
IMPORTANTE (runtime):
- La hora local ya está resuelta por el servidor. Ahora estamos **${estadoAhora}**.
- Si preguntan “¿están abiertos ahora?”, respondé con una línea corta usando ese estado (por ejemplo: “Ahora estamos ${estadoAhora}.”) y, si aplica, agregá los horarios (“Abrimos de 8 a 21 hs, de lunes a sábados.”).
...
- Pagos: efectivo, tarjetas, Mercado Pago [link](https://biolibre.ar/lavanderiaburbujas), Cuenta DNI, Bitcoin.  
        ...
        Ten siempre en cuenta el horario actual y el horario comercial, pero no lo menciones explícitamente.
        ...
        Efectivo, débito, crédito, Mercado Pago, Cuenta DNI, +Pagos Nación, Bitcoin (red Bitcoin o a través de la Lightning Network). Solicitar QR a Burbujas.
        ...
        20% de reintegro de lunes a viernes, con un tope de 8.000 pesos por mes y persona.
        ...
        El ganador del sorteo lavados gratis para el mes de mayo 2025 fue Luis Alvarez (4 lavados de ropa gratis y 2 lavados de acolchados). 
        Ganadora de la promoción sorteo: Alejandra Sosa (2 entradas entre los clientes para ver a María Becerra el 22 de marzo de 2024 en el Estadio River Plate).
        Ganador de la Promoción "6 meses de lavados gratis": Martín Acuña (31 de mayo de 2024).
...
- Si pregunta dónde se anunciará el ganador → En redes sociales de Burbujas.
        ...
        Burbujas está armando una sucursal de nuestra lavandería en el Parque Termal Dolores. 
        En desarrollo y construcción para satisfacer las necesidades de servicio de lavandería de los usuarios, así como ofrecer servicio a cabañas y hoteles fortaleciendo las capacidades y servicios. 
        Burbujas no administra el parque termal. 
        Cualquier información sobre el parque termal debe enlazar siempre a https://www.termasdolores.com.ar/
        El sitio web del parque termal no es burbujas.online. Atención.
        ...
        Procura que los mensajes que contengan pedidos para delivery siempre soliciten la dirección y un horario cómodo para el cliente y que lo envíen por [WhatsApp](https://wa.me/5492245402689) o a [Instagram](https://www.instagram.com/burbujasdolores/) o a [Telegram](https://t.me/Burbujas_lavanderia)
        Contacto:
        ...
        Lavandería Burbujas tiene esta Playlist: Descubrí tu Flow en Spotify → https://www.burbujas.online/playlist-de-lavanderia
        También tiene una playlist de Rock nacional en Spotify que se llama "Rock de acá" → https://www.burbujas.online/rock-argentino
        En Burbujas creamos "Ai Vibra" y hacemos música con IA para que nuestros clientes puedan disfrutarla en todas las plataformas. [Ai Vibra] https://open.spotify.com/intl-es/artist/3L4WxpiMyJ7aNIiCmWL0Hl?si=JhIbIRKmQO-Qc1_58rOgRw
        Burbujas cuenta en su web con todos los eventos y recitales para que el público esté al corriente. Sugerí que naveguen en [Descubrí tu Flow] https://www.burbujas.online/playlist-de-lavanderia
...
`.trim();
    ...
    // No mostrar "(Arg)" ni frases similares
    reply = reply
      .replace(/\s*\(arg\)\s*/gi, " ")
